  ...
  function(oj, ko, $, moment) {
    ...
    function ControllerViewModel() {
      ...
      function getDisplayedDate(date) {
        ...
        switch (self.date.period()) {
          case 'day': {
            return wrappedDate.format('DD.MM.YYYY');
          }
          break;
          case 'week': {
            return wrappedDate.format('YYYY') + 'W' + wrappedDate.format('WW');
          }
            break;
          case 'month': {
            return wrappedDate.format('YYYY MM');
          }
            break;
          case 'year': {
            return wrappedDate.format('YYYY');
          }
            break;
        }
      }
      ...
      self.appName = ko.observable("Test");

      var defaultMarket = {code: 'TOTAL', name: '(000) ALL MARKETS'};
      self.market = ko.observable(defaultMarket);

      self.markets = ko.observableArray([
        {code: 'TOTAL', name: 'ALL MARKETS'},
        {code: '001', name: '001'},
        {code: '002', name: '002'}
      ]);

      function standardChangeHandler(data, itemDestiny, itemsList) {
        if (data.option === 'value') {
          for (i in itemsList()) {
            var item = itemsList()[i];
            if (item.code === data.value[0]) {
              itemDestiny(item);
              break;
            }
          }
      ...
      self.marketChangeHandler = function(event, data) {
        standardChangeHandler(data, self.market, self.markets);
        self.segment(defaultSegment);
        self.category(defaultCategory);
        self.family(defaultFamily);
      };

      // Segments

      var defaultSegment = {code: 'TOTAL', name: '(000) ALL SEGMENTS'};
      self.segment = ko.observable(defaultSegment);

      self.segments = ko.observableArray([
        {code: 'TOTAL', name: 'ALL SEGMENTS'},
        {code: '001', name: '001'},
        {code: '002', name: '002'}
      ]);

      self.segmentChangeHandler = function(event, data) {
        standardChangeHandler(data, self.segment, self.segments);
        self.category(defaultCategory);
        self.family(defaultFamily);
      ...
      var defaultCategory = {code: 'TOTAL', name: '(000) ALL CATEGORY'};
      self.category = ko.observable(defaultCategory);

      self.categories = ko.observableArray([
        {code: 'TOTAL', name: 'ALL CATEGORIES'},
        {code: '001', name: '001'},
        {code: '002', name: '002'}
      ]);

      self.categoryChangeHandler = function(event, data) {
        standardChangeHandler(data, self.category, self.categories);
        self.family(defaultFamily);
      ...
      var defaultFamily = {code: 'TOTAL', name: '(000) ALL FAMILY'};
      self.family = ko.observable(defaultFamily);

      self.families = ko.observableArray([
        {code: 'TOTAL', name: 'ALL FAMILIES'},
        {code: '001', name: '001'},
        {code: '002', name: '002'}
      ]);
      ...
      self.periodTitle = ko.pureComputed(function() {
        // Do it to force recomputing of the title every time localization is loaded
        var loaded = self.isLocalizationLoaded();
        return capitalize(oj.Translations.getTranslatedString('period from')) + ' ' + getDisplayedDate(self.date.first()) + ' ' +
          oj.Translations.getTranslatedString('to') + ' ' + getDisplayedDate(self.date.second());
      });

      self.nomenclatureTitle = ko.pureComputed(function() {
        function formatForTitle(item) {
          return item.code !== 'TOTAL' ? item.name : '';
        }

        var loaded = self.isLocalizationLoaded();
        return capitalize(oj.Translations.getTranslatedString('nomenclature')) + ' ' +
          self.market().name + ' ' + formatForTitle(self.segment()) + ' ' +
          formatForTitle(self.category()) + ' ' + formatForTitle(self.family());
      });
      ...
      var nomenclature = {
        ...
        marketOptionChange: self.marketChangeHandler,
        segment: self.segment,
        segments: self.segments,
        segmentOptionChange: self.segmentChangeHandler,
        category: self.category,
        categories: self.categories,
        categoryOptionChange: self.categoryChangeHandler,
        family: self.family,
        families: self.families,
        familyOptionChange: self.familyChangeHandler,
        title: self.nomenclatureTitle
      };

      self.moduleParams.main = {
        lang: self.lang,
        date: self.date,
        periodTitle: self.periodTitle,
      ...
      self.api2 = {
        protocol: 'http',
        server: 'auth.navitek.com.ru',
        port: '80',
        service: 'restapi/v.1/offer'
      };

      self.api2.report = self.api2.protocol + '://' + self.api2.server + ':' + self.api2.port + '/' + self.api2.service;
      self.api2.referenceByTier = self.api2.report + '/referencebytier';

      function getTierData() {
        var url = self.api2.referenceByTier,
          params = { tokenid: self.user.tokenId()};
        ...
        var jqxhr = $.getJSON(url, params)
          ...
          .fail(function(reason) {
            oj.Logger.error('fail');
            oj.Logger.log(reason);
          })
          .always(function(data) {
            oj.Logger.log(this.url);
          });
      ...
      self.getPagePath = function(path) {
        return 'pages/' + path + '/' + path.substr(path.lastIndexOf('/') + 1);
      }

      self.router = oj.Router.rootInstance;
      self.router.configure({
        'login': {value: self.getPagePath('login'), label: 'Login', isDefault: true},
        'nomenclature': {value: self.getPagePath('nomenclature'), label: 'Nomenclature', enter: function() { getTierData(); }},
        'tier': {value: self.getPagePath('tier'), label: 'Tier', enter: function() {}}
        // 'family': {value: self.getPagePath('family'), label: 'Family', enter: function() {}},
        // 'ctm': {value: self.getPagePath('ctm'), label: 'CTM', enter: function() {}}
      });
      oj.Router.defaults['urlAdapter'] = new oj.Router.urlParamAdapter();

      self.firstPage = 'nomenclature';

      oj.Logger.log(self.router);
      ...
      self.sidePanelParams = ko.observable();
